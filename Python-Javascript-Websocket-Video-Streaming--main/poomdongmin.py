...
@sio.on('chat')
async def on_chat(sid, message):
    target_room = rooms_sid.get(sid)
    await sio.emit('chat', message, room=target_room)
    print(f"Message received from {sid} in room {target_room}: {message}")
